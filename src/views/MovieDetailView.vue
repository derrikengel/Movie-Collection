        ...
        <div v-if="movie" :class="s.page">
            ...
            <div :class="s.contentWrap">
                ...
                <div :class="s.content">
                    <div :class="s.wideCol1">
                        ...
                        <MovieRequestBar v-if="isRequest" :request="movie" />
                        <template v-else>
            ...
            <div :class="[s.browse, sourceListClass]">
                <div :class="s.browseNav">
                    <RouterLink v-if="prevMovie"
                        :to="{ name: isRequest ? 'request' : 'movie', params: { slug: prevMovie.slug } }"
                        :class="[s.browseLink, s.browseLinkPrev]">

                        <span v-html="arrowLeft" :class="s.browseIcon" />

                        <div :class="s.browseContent">
                            <img v-if="prevMovie.poster_path" :src="posterUrl(prevMovie.poster_path)"
                                :alt="prevMovie.title" :class="s.browsePoster" />
                    ...
                    <RouterLink v-if="nextMovie"
                        :to="{ name: isRequest ? 'request' : 'movie', params: { slug: nextMovie.slug } }"
                        :class="[s.browseLink, s.browseLinkNext]">
                        <div :class="s.browseContent">
                            <img v-if="nextMovie.poster_path" :src="posterUrl(nextMovie.poster_path)"
                                :alt="nextMovie.title" :class="s.browsePoster" />
...
<script setup>
    ...
    const isRequest = computed(() => !!route.meta.isRequest)

    const movie = computed(() => {
        if (isRequest.value) return requestsStore.requests.find(r => r.slug === route.params.slug) ?? null
        return moviesStore.movies.find(m => m.slug === route.params.slug) ?? null
    })
    ...
    const notFound = computed(() => {
        if (isRequest.value) return requestsStore.fetched && !requestsStore.loading && movie.value === null
        return !moviesStore.loading && movie.value === null
    })
    ...
    const sourceLabel = computed(() => {
        if (isRequest.value) return 'Requests:'
        return sourceLabelMap[navContext.sourceList] ?? ''
    })
    ...
    const browseList = computed(() => {
        if (isRequest.value) return requestFilters.filteredRequests
        return navContext.contextMovies.length ? navContext.contextMovies : filters.filteredMovies
    })

      ...
      >
        <div :class="s.listCardHeader">
          <div :class="s.listCardMeta">
            <span :class="s.listCardIcon" v-html="list.icon" aria-hidden="true" />
            <span :class="s.listCardName">{{ list.label }}</span>
          </div>
          <span :class="s.listCardCount">{{ list.count }}</span>
        </div>

        <div :class="s.listCardPosters">
          <template v-if="list.previews.length">
            <img
              v-for="movie in list.previews"
              :key="movie.id"
              :src="`https://image.tmdb.org/t/p/w92${movie.poster_path}`"
              :alt="movie.title"
              :class="s.listCardPoster"
            />
            <div v-if="list.count > list.previews.length" :class="s.listCardMore">
              +{{ list.count - list.previews.length }}
            </div>
          </template>
